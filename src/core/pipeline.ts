...
/**
 * 量化映射的单遍实现：为每格选最近色。抖动在这一步内完成（见文件头约束 2）。
 *
 * - 关抖动时启用 RGB→索引的直接映射缓存（≤2^15 槽 = 每通道 5 bit，共 32768 槽）：
 *   照片里大量重复色能省掉重复的 OKLab 比对。
 * - 开抖动时**必须关闭缓存**：F-S 扩散后每格的实际输入色带累计误差，缓存会算出错误结果。
 *
 * 色号上限**不在这里处理**——它是 `quantize` 的两遍法，见那边的说明。
 * 本函数返回的就是普通的 `{ indices, overflow }`，没有更丰富的报告结构。
 */
 ...
 *
 * 不做色号上限时就是一遍 `quantizeOnce`；启用上限（`cap >= 2` 且开了抖动）时走
 * 下面的**两遍法**（`ditherMaxColors` 的语义见 `limits.ts` 与 `spec.ts`）。
 *
