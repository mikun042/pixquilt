...
- **刷新不丢编辑**：自动草稿兜底，重开时问你要不要恢复（见「已知边界」里的说明）。
...
src/core/                ← 纯逻辑：零 DOM、零 node: 依赖，Node 可直接 import
   ...
   pipeline.ts           ←   像素化管线
   ops.ts rasterize.ts   ←   13 类编辑算子 / 几何栅格化（+ canvas-query、ops-shapes）
...
src/app/                 ← 浏览器侧：UI、画布、页内 API（automation.ts）、预设（presets.ts）、
                            导出动作（export-actions.ts）、撤销栈（history.ts）、自动草稿（storage.ts）
...
tool/bench.mjs           ← 性能基准（零依赖、不起浏览器；比值类结论带断言，耗时只作参考）
tool/icons/              ← UI 图标管线：形状定义 → icons.ts 的两张 path 表（见其 README）
docs/                    ← 见下方「文档导航」
```
...
> 数字会过期，以命令输出为准。`npm run bench`（或 `--quick`）跑性能基准——
> 它只把**比值**当断言，绝对耗时随机器浮动、不作验收标准。改 UI 外观另有两个工具：
...
- **自动草稿只保"最近一次"**（刷新后弹「恢复 / 放弃」提示条）：它防手滑，不是归档；
  很大的原图可能因浏览器配额存不进去（此时画布照常恢复，只是不能重新转换）。要长期保存请导出项目 JSON。
