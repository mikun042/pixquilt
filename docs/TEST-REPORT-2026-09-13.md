...
## 九、第二轮：外部 agent 工具使用反馈（`output/工具问题记录.md`）

另一位 agent 用 CLI 产出 3 张精灵后提交了 8 条问题。逐条核实后发现：
**7 条成立、1 条归因不成立**，另外查出 4 条它没提到的问题。完整回执见
`output/工具问题记录-修复回执.md`，此处只记录结论与改动。

### 9.1 处置总表

| 编号 | 报告主张 | 核实 | 处置 |
|---|---|---|---|
| P1-1 | 默认 cleanup 吃掉 1–2px 细节 | **成立** | `runPipeline` 增结构化 `cleanup` 报告；CLI 显式警告点名 hex 与格数 |
| P1-2 | `--blank` 无条件写拼豆 SVG+CSV | **成立**（`if (beadingOptions)` 恒真） | 改 `if (args.bead)` |
| P2-1 | 目录里的 `.svg` 让整批 exit 1 | **成立**，且与 `--in file.svg` 行为不一致 | 预筛，记入 `skippedFiles` |
| P2-2 | `--blank` 无 `.hex`/`.json`，`--sheet` 只在 `--in` 内 | **成立** | 抽出共用；顺带补 `transparent` 字段 |
| P2-3 | 降采样不可用，建议拒绝或改整数倍最近邻 | **归因不成立** | 未按建议改，理由见 9.2 |
| P3-1 | 无 `--ops @file` | 成立 | 加 `--ops-file` / `--ops @f` |
| P3-2 | `--blank` 硬编码 `index:1` 撞名 | **成立**，且真相比报告更严重 | 修 `{index}`；并修掉自引用导致的占位符残留 |
| P3-3 | 缺 `outline`/`mirror` 算子 | 成立 | **不采纳**（会改公开 API，语义需先定；`--ops-file` 已解痛点） |

报告未提到、本轮查出并修复的：**未知参数静默忽略**（`--exact 32x32` 照常"成功"）、
**`--json` 的 stdout 不是纯 JSON**（首字符 `✔`，`jq` 直接失败）、
**`artc.mjs` 被 import 时执行 `main()`**（会把导入方一起 `process.exit`）、
**帮助文本漏掉 3 个已实现参数**。详见 `docs/ARCHITECTURE.md` §8.5–8.7。

### 9.2 唯一未采纳的建议，以及为什么不采纳

报告认为"64→32 降采样不可用"，处方是"拒绝降采样"或"实现保描边的整数倍 nearest"。实测两者都不必做：

- **整数倍下 `nearest` 已经无损**：512→64（真 8:1）实测描边 132 格，与 1:1 基线**完全一致**。
- **真 2:1 下两种模式的差别是"混合色"而非"丢描边"**：`average` 造出的 8 个新色
  （`#8c8c96`、`#ee7488`…）是描边与背景按面积混合的结果，即区域平均的定义，不是缺陷。
- **带 alpha 的精灵再测一遍**：`nearest` 与 `average` 保留的结构完全相同，alpha 阈值没有额外吃掉细描边。

真正的可用性问题是**默认值对像素素材不友好**（默认 `average` + `cleanup` 开），
这与 P1-1 同源。已按同一原则处理：**先如实上报，不擅自改默认行为**——
改默认会静默改变既有批次的产出，属于需要需求方决定的事。可靠写法已写进
`docs/AGENT-QUICKSTART.md` 第四节（显式给 `--downsample nearest --no-cleanup --palette-k 64`）。

### 9.3 回归防线（本轮新增）

每条修复都配了**能因缺陷变红**的断言，并用变异测试确认有效性：

| 变异 | 断言表现 |
|---|---|
| `cleanup` 报告恒为 `null` | ✘ 启用 cleanup 时必须给出报告，不能是 null |
| `removedColors` 恒为空数组 | ✘ 被整幅吃掉的颜色必须出现在报告里 |
